...
class BdTrans
{
    ...
    public function slug($query,$from='auto',$to = 'en')
    {
        $data = $this->translate($query, $from , $to );
        $data = $data['trans_result'][0]['dst'];
        return $this->sluggable($data);
    }
...
//翻译入口
    ...
    public function translate($query, $from='auto', $to='en')
    {
        ...
        $args['sign'] = $this->buildSign($query, $this->config["appKey"], $args['salt'], $this->config["appSecret"]);
